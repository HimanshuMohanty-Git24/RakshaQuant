"""
The shared live-trading session.

``run_trading_session`` is the *single* trading loop for the whole system. It was extracted
verbatim from the legacy ``scripts/run_live_trading.py`` and parameterised by a
:class:`~src.live.views.SessionView`, so the CLI (``RichSessionView``) and the web UI
(``StreamSessionView``) drive identical trading logic — no second, divergent loop.

**Do not regress** any of the safety invariants documented in ``CLAUDE.md`` (kill-switch
gates execution not just the graph; idempotent submits; shadow default-off; data-staleness
gate; drawdown seed from real equity; net-of-cost P&L). The order of engine calls here is
the order the CLI has always used.
"""
...
async def run_trading_session(
    ...
    *,
    ...
    """
    Run the live/simulated trading loop, rendering through ``view``.

    Args:
        view: renderer (``RichSessionView`` for CLI, ``StreamSessionView`` for web).
        stop_event: when set, the loop finishes the current wait and exits cleanly
            (used by the web server to stop a run without a KeyboardInterrupt).
        max_cycles: optional cap on cycles (tests / bounded runs); ``None`` = forever.
    """
    settings = get_settings()

    # Bind a TradingDashboard to the view's stats object so the loop's mutation helpers and
    ...
    # limb. Seed from the engine's actual equity (it may have RESTORED a persisted
    # wallet/positions on startup), not the static configured balance.
    ...
    async with view:
        ...
        async def _run_cycle(cycle: int) -> None:
            ...
            # Trace capture (web only): snapshot FinOps by-agent before/after so each span
            # carries its own token/cost delta. No-op when the view has no recorder (CLI).
            recorder = view.recorder
            if recorder is not None:
                recorder.begin(cost_tracker.daily_summary().get("by_agent", {}))
            ...
            if recorder is not None:
                trace = recorder.finish(
                    ...
                    by_agent_after=cost_tracker.daily_summary().get("by_agent", {}),
                )
                await view.emit_cycle(trace)
            ...
            # Kill-switch gate: a daily-loss / drawdown breach must halt NEW entries at the
            # point of execution (exits in Step 0 still run, to flatten risk).
            kill_state = {
                "daily_stats": daily_stats,
            ...
            for trade in approved:
                ...
                # Circuit guard: don't open into a scrip at/through its NSE price band.
                ...
                # Risk-based position sizing from the risk budget + stop distance (Kelly when a
                # strategy win-rate exists) — never a flat % of cash.
                ...
                if result.filled:
                    ...
                    # Remember which lessons were active at entry, to score them at close.
            ...
            if goal_report.get("enabled"):
                ...
                if not goal_report["feasible"]:
                    ...
                    await get_alert_manager().alert(
                        "goal_off_pace",
                        f"Behind profit pace: month-to-date "
                        f"Rs.{goal_report['month_to_date_pnl']:,.0f} "
        ...
        try:
            ...
            view.note("[yellow]Trading stopped[/]")
            view.note(
